...
namespace Model.Helper
{
    ...
    {
        #region 雪花算法
        private static long workerId = 100; //机器ID
        ...
        public static long nextId()
        {
            lock (lockObj)
            {
                ...
                { //同一微妙中生成ID
                    Helper.sequence = (Helper.sequence + 1) & Helper.sequenceMask; //用&运算计算该微秒内产生的计数是否已经到达上限
                    if (Helper.sequence == 0)
                    {
                ...
                long nextId = (timestamp - twepoch << timestampLeftShift) | Helper.workerId << Helper.workerIdShift | Helper.sequence;
                return nextId;
            }
        }
